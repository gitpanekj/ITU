<script lang="ts">
  ...
  import { teacherView } from '../../../stores/Reading/teacherView';
	...
	const saveText = async () => {
		...
		const response = await fetch(`http://localhost:3000/reading-exercise/text/${readingId}`,
			{
				method: 'PATCH',
				headers: {
                	'Content-Type': 'application/json', // Indicate that the payload is JSON
            	},
				body: JSON.stringify({text: {type: content?.type, content: content?.content}}),
			}
		);
	}
	...
	onMount(async () => {
		...
		editor = new Editor({
			...
			editorProps: {
				attributes: {
					class: 'pt-8 px-8 bg-white border-solid border-2 border-slate-300 focus:outline-none border-2 border-black overflow-y-auto h-full prose-lg'
				}
			},
			injectCSS: false,
			onTransaction: () => {
...
<div class='flex flex-col' style="height: calc(100vh - 4em)">
	{#if editor}
	<div class="flex flex-row bg-slate-400 h-16 justify-between items-center px-8">
		...
		<div class="flex flex-row gap-8">
			...
			<div class="flex flex-row gap-1 items-center">
				<button
					on:click={() => {console.log('H1');editor?.chain().focus().toggleHeading({ level: 1 }).run()}}
		...
		{:else}
			<button
			on:click={async () => {await editor?.commands.setCustomHighlight(1); console.log(editor?.getHTML()); teacherView.view_mode()}}
